...
@Component({
  selector: 'app-supplier-ageing-report',
  templateUrl: './supplier-ageing-report.page.html',
  styleUrls: ['./supplier-ageing-report.page.scss'],
  encapsulation: ViewEncapsulation.None
})
export class SupplierAgeingReportPage implements OnInit {
    ...
    getReport(){
      ...
      this.GetSupplierAging(this.datepipe.transform(this.Fromd,'dd/MM/yyyy'),
      this.datepipe.transform(this.Tod,'dd/MM/yyyy'),this.CustId,this.SctId)
      
     
      // this.GetBillwiseSale(this.datepipe.transform(this.Fromd,'dd/MM/yyyy'),this.datepipe.transform(this.Tod,'dd/MM/yyyy'))
    }
    async GetSupplierAging(fd: any, td: any,custid:any,SctId:any) {
      ...
      this.repser.GetSupplierAging(fd, td,custid,SctId).subscribe(
        (data: any) => {
         loading.dismiss();
          console.log(data);
    ...
    openModal(items:any)
    {
      ...
      this.GetSupplierAgingDtls(this.datepipe.transform(this.Fromd,'dd/MM/yyyy'),this.datepipe.transform(this.Tod,'dd/MM/yyyy'),items.VENDOR_ID,this.SctId)
  
          
    }
  
    async GetSupplierAgingDtls(fd: any, td: any,custid:any,SctId:any) {
      const loading = await this.loadingCtrl.create({
        cssClass: 'custom-loading',
        message: 'Loading...',
        spinner: 'dots',
      ...
      this.repser.GetSuppAgingDtls(fd, td,custid,SctId).subscribe(
        (data: any) => {
          loading.dismiss();
          console.log(data);
    ...
    async GetCustomer(fromDate:any,toDate:any,custid:any,SctId:any){
      ...
      await loading.present();
      this.repser.GetSupplierAging(fromDate,toDate,custid,SctId).subscribe((data:any)=>{
        loading.dismiss()
       console.log(data);
        ...
        if(data.Status==200)
        {
          if(data.Data.length>0)
          {
            for(let i=0;i<data.Data.length;i++)
              {
                if(!this.VendorsList.some((x:any)=>x.VENDOR_ID==data.Data[i].VENDOR_ID))
                {
                  let obj = {VENDOR_ID:data.Data[i].VENDOR_ID,VENDOR_NAME:data.Data[i].VENDOR_NAME}
                  this.VendorsList.push(obj)
                  this.filteredVendorsList.push(obj)
  ...
  // VendorsList: any[] = []; // Original customer list
  ...
  exportToExcel(): void {
    const rawTitle =' Supplier Ageing Report from ' +
    ...
    const header = ['Sl.No', 'Supplier', '30', '60', '90', '120', 'ABOVE 120'];
  ...
  exportToPDF(): void {
    ...
    const columns = ['#', 'Supplier', '30', '60', '90', '120', 'ABOVE 120'];
    const rows = this.BillWiseReportData.map((item, index) => [
          index + 1,
          item.VENDOR_NAME,
          item["'30'"],
  ...
  getTotalForMonth(monthKey: string): number {
      ...
      this.repser.GetDepartment().subscribe((data:any)=>{
        ...
        if(data.Status==200)
        {
          ...
          // this.GetAllUserRightsTab(data.MenuGroupId)
            ...
            // this.filteredVendorsList=data.Data
            console.log(this.VendorsList,this.filteredVendorsList);
          }
          else
          {
            //  this.VendorsList=[]
            // this.filteredVendorsList=[]
            this.sctList=[]
          //  this.BillWiseReportData = [];
          }
   ...
   async OpencustomerModal(){
    const modal = await this.modalController.create({
      ...
      componentProps: {
        DropDownData:this.VendorsList,
        Headers:['Supplier'],
        Fields:['VENDOR_NAME'],
        ModalHeader:'Suppliers List',
        CheckType:'check',
        UniqueField:'VENDOR_ID',
        SelectedItems:this.SelectedVendors
    ...
    const { data, role } = await modal.onWillDismiss(); // You can also use onDidDismiss()
      ...
      if (role === 'confirm') {
        ...
        this.SelectedVendors=data.selected
        console.log(values);
        // this.CustomersName = values['Customer Name'];
  
        this.VendorsName = values.map((item:any) => item.VENDOR_NAME);
        console.log(this.VendorsName);
        let custid = values.map((v:any) => v.VENDOR_ID).join(',');
        this.CustId=custid
        console.log(custid);
